...
class HtmlDrawerUtils {
     ...
     */
    public static createButton(innerText: string, isEnabled: boolean, func: any): HTMLButtonElement {
        let resultButton: HTMLButtonElement = document.createElement('button');
     ...
     */
    public static controlButton(button: HTMLButtonElement, isEnabled: boolean) {
     ...
     */
    public static createRacersNameDiv(racersName: string[]): HTMLDivElement {
        let racersNameDivResult: HTMLDivElement = document.createElement('div');
        racersNameDivResult.innerText = 'The racers are: ';
        racersName.forEach((name: string) => {
     ...
     */
    public static createCountDownDiv(countDownCounter: number): HTMLDivElement {
        let countDownDivResult: HTMLDivElement = document.createElement('div');
     ...
     */
    public static buildHorseField(fieldLength: number, horse: Horse): HTMLTableRowElement {
     ...
     */
    private static createHorseNameCell(horse: Horse): HTMLTableDataCellElement {
        let resultDataCell: HTMLTableDataCellElement = document.createElement('td');
     ...
     * @returns {HTMLTableDataCellElement} The cell element instance
     */
    private static createHorseFieldCell(horse: Horse, fieldLength: number): HTMLTableDataCellElement {
        let resultDataCell: HTMLTableDataCellElement = document.createElement('td');
        let horseDiv: HTMLDivElement = document.createElement('div');
        let horsePictureHtml: HTMLImageElement = document.createElement('img');
...
class RaceField {
    ...
    private highlightFlag: boolean;
    private countDownCounter: number;
     ...
     */
    constructor(fieldLength: number, countdownStart: number, horses: Horse[]) {
        ...
        let fieldTable: HTMLTableElement = document.createElement('table');
        ...
        this.startButton = HtmlDrawerUtils.createButton('Start Race!', true, () => this.startCountDown());
        this.resetButton = HtmlDrawerUtils.createButton('Reset Race!', false, () => this.resetRace());
     ...
     */
    private checkForWinner(horse: Horse) {
     ...
     */
    private highlightWinner(horse: Horse) {
    ...
    private startCountDown() {
        ...
        this.startTimer = setInterval(() => {
            ...
            if(this.countDownCounter == 0) {
                this.countDownCounter = this.countDownStart;
                clearInterval(this.startTimer);
                this.countDownDiv.innerText = 'Start!';
                this.startRace();
            }
        }, 1000);
    }
     ...
     */
    private getRacersName() {
        return this.horses.reduce((names: string[], horse: Horse, currentIndex: number, array: Horse[]) => {
